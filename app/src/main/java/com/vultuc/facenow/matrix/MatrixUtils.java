package com.vultuc.facenow.matrix;
...
public class MatrixUtils {
    ...
    public static int[] readImg(String path) throws IOException {
        File f = new File(path);
        BufferedImage img = ImageIO.read(f);
        img = resize(img, 100, 100);
        int[][] m = convertTo2D(img);

        int[] result = new int[m.length * m[0].length];
        for(int i = 0; i < m.length; i++){
            for(int j = 0; j < m[0].length; j++)
            {
                result[i*m[0].length + j] = m[i][j];
            }
        }
        return result;
    }
    */
    ...
    public static int[] readImgB(Bitmap bmH){
        Bitmap bm = Bitmap.createScaledBitmap(bmH,  100,
                100, false);
        int[] result = new int[bm.getWidth() * bm.getHeight()];
        for(int x = 0; x < bm.getWidth(); x++){
            for(int y = 0; y < bm.getHeight(); y++){
                // получим каждый пиксель
                int pixelColor = bm.getPixel(x, y);
                // получим цвет каждого пикселя
                int pixelRed = Color.red(pixelColor);
                int pixelGreen = Color.green(pixelColor);
                int pixelBlue = Color.blue(pixelColor);
                result[x*bm.getHeight() + y] = (pixelRed + pixelGreen + pixelBlue)/3;
            }
        }
        return result;
    }
...
/*
    ...
    public static int[][] convertTo2D(Bitmap image){
        int width = image.getWidth();
        int height = image.getHeight();
        int[][] result = new int[height][width];
        image.copyPixelsToBuffer();
        Raster raster = image.getData();
        for (int j = 0; j < width; j++) {
            for (int k = 0; k < height; k++) {
                result[j][k] = raster.getSample(j, k, 0);
            }
        }
        return result;
    }
